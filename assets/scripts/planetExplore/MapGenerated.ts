import { _decorator, Component, Node, Enum, Vec2, Prefab, instantiate, Vec3, RichText, Label, EventHandler, Button } from 'cc';
...
export class MapGenerated extends Component {
    ...
    DestroyMap() {
        ...
        MoveController.GetInstance().ResetMove();
    }

    SetLevel(event: Event) {
        const node = event.target as unknown as Node;
        const level = node.getSiblingIndex();
